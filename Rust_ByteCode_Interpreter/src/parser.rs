...
pub(crate) fn parse_code() -> (HashMap<String,u8>, Vec<Operation>) {
    ...
    let mut instruction: Vec<&str> = buf.split('\n')
                        // split lines
                         .map(|i| i.trim())
    ...
    //match all operations to this block slice and push them into operations map
        for i in instruction { 
            let block: Vec<&str> = i.split(|c| c == ' ' || c == ',')
                                    .filter(|c| c.len() > 0)
                                    .collect();
            let operation = match &block[..] {
                ...
                &["SEND_CHANNEL", name] => Operation::SEND_CHANNEL(parse_var(name)),
                &["RECV_CHANNEL", name] => Operation::RECV_CHANNEL(parse_var(name)),
